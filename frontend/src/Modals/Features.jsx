...
const Features = () => (
  /* 
    w-[min(90vw,480px)] — never wider than viewport on mobile,
    caps at 480px on desktop. 
  */
  <div className="w-[min(90vw,480px)] rounded-2xl border border-gray-200 bg-white p-3 shadow-xl">
    <ul className="grid grid-cols-1 sm:grid-cols-2 gap-1">
      {featuresData.map((item) => (
        <li key={item.title}>
          <Link
            to={item.link}
            className="flex items-start gap-3 rounded-xl p-3 transition hover:bg-gray-50 group"
          >
            {/* Icon chip */}
            <span className="mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-gray-100 text-gray-500 transition group-hover:bg-white group-hover:shadow-sm">
              {icons[item.title]}
            </span>
